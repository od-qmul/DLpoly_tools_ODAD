...
def get_new_lat():
    #Open file resizecon
    lat_file=open("resizecon",'r')
    #extract lines of CONFIG file
    lines = lat_file.readlines()
    ...
    new_lat_par=[[0,0,0],[0,0,0],[0,0,0]]
    i=0
    for line in lines[0:4]:
        line_latpar = line.split()
        lat_par_temp=line_latpar[0:3]
        lat_par_temp=[float(element) for element in lat_par_temp]
        new_lat_par[i]=lat_par_temp
        i=i+1
    return new_lat_par
...
new_latpar=get_new_lat()    
config=Read_config("CONFIG")

final_coord = get_new_coords(config.frac_data,new_latpar)

write_config("newCONFIG",final_coord,new_latpar)

#for line in final_coord:
#    print(line)
#for line in config.frac_data:
#    print(line)
