...
class get_input:
    def __init__(self):
        ...
        self.Natoms=0
        for element in self.atom_dat:
            self.Natoms=self.Natoms+int(element[2])
        #define distamce between grid point
        self.grdsize=int((np.cbrt(float(self.Natoms))+1.0))
        self.grdlen=lat_length/float(int((np.cbrt(float(self.Natoms))+1.0)))
        self.grdfrac=1.000/float(int((np.cbrt(float(self.Natoms))+1.0)))
...
a=random.sample(range(indat.grdsize),indat.grdsize)
b=random.sample(range(indat.grdsize),indat.grdsize)
c=random.sample(range(indat.grdsize),indat.grdsize)
print(a)
print(b)
print(c)
gridpos=[]
for i in range(len(a)):
  for j in range(len(b)):
      for k in range(len(c)):
        gridpos.append([indat.grdfrac*float(a[i]),indat.grdfrac*float(b[j]),indat.grdfrac*float(c[k])])
  
pointselect=random.sample(range(indat.Natoms),indat.Natoms)
print(gridpos)
...
for element in indat.atom_dat:
    for i in range(int(element[2])):
        randcoord=gridpos[pointselect[j]]
        j=j+1
        this_atom=[]
        this_atom.append(element[0])
        this_atom.append(j)
        this_atom.append(randcoord)
