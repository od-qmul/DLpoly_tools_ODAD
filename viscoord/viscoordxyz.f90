...
read(50,*)
do i = 1, numatoms
   read(50,Fmt='(i12,1x,A8,i12)',advance='no')indx,atm1,coord
        ...
        if(coordpairs(ii,1).eq.atm(indx))then
          ...
          coordpair=ii
           if(perfectcoord(coordpair).ne.coord)then
             ...
             read(50,*)(tempindx(iii),iii=1,coord),(tempatm(jj),jj=1,coord)
              do jjj= 1,coord
                if(tempatm(jjj).eq.coordpairs(ii,2))then
!                   numdefects2=numdefects2+1
!                   defectindx2(numdefects2)=tempindx(jjj)
                endif
              enddo
           else
             read(50,*)
...
write(110,*)title
do i =1,ndefs
write(110,*)atm(truedefects(i)),x2(defects(i)),y2(defects(i)),z2(defects(i))
enddo
