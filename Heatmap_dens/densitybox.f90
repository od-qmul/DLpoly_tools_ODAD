...
implicit none 
integer :: densatoms,i,ii,iii,j,jj,jjj,k,kk,kkk,numatoms,ignore,boxlen,numinx,numiny,numinz,shift
character( len= 8) :: atomtypes(1:3000),atmtypes(1:3000)
character( len= 80) :: line
real, allocatable :: mass(:),x(:),y(:),z(:)
real :: x0,x1,y0,y1,z0,z1,xmid,ymid,zmid,totalmass,v(1:3,1:3),density,volume,amu,a,b,c
amu=1.66054e-27
  ...
  do jj=1,numiny
   do kk=1,numinz 
    totalmass=0
     ...
     y0=(-(b/2.0)+jj*shift)
     z0=(-(c/2.0)+kk*shift)
     x1=(-(a/2.0)+10.0+ii*shift)
     y1=(-(b/2.0)+10.0+jj*shift)
     z1=(-(c/2.0)+10.0+kk*shift)
     xmid=X0+boxlen/2
     ymid=y0+boxlen/2
     zmid=z0+boxlen/2
     do i=1,numatoms
     ...
     density=(totalmass*amu/volume)
     write(30,*)numinx*numinz*numiny,density,xmid,ymid,zmid
   enddo
  enddo
 enddo
